...
## 敌人
var enemy:Character000Base = null
## 检测到敌人后的移动时间
...
func init_bullet(bullet_paras:Dictionary[E_InitParasAttr,Variant]):
	...
	if not is_instance_valid(enemy):
		enemy = detect_component_global.update_enemy_track_bullet()


func _process(delta: float) -> void:
	## 如果敌人死亡或敌人不存在
	if (is_instance_valid(enemy) and enemy.is_death) or not is_instance_valid(enemy):
		enemy = detect_component_global.update_enemy_track_bullet()
		current_time = 0
		if not is_instance_valid(enemy):
...
func set_next_point(delta):
	current_time += delta
	var distance = global_position.distance_to(enemy.global_position + target_pos_correct)
	...
	next_point = global_position.bezier_interpolate(start_control_point, enemy.global_position + target_pos_correct, enemy.global_position + target_pos_correct, t)
...
func _on_area_2d_attack_area_entered(area: Area2D) -> void:
	...
	if max_attack_num != -1 and curr_attack_num < max_attack_num:
		if area.owner == enemy:
			attack_once(enemy)
	## 子弹无限穿透 TODO:这地方会有问题,不过一般没有无限穿透的追踪子弹
	if max_attack_num == -1:
		if area.owner == enemy:
			attack_once(enemy)
